...
bool	server_open(char *host, int port, t_server *server)
{
	...
	users_teardown(server->users);
	LOG_DEBUG("listen %d home {%s}", server->listen, home);
	INIT_LIST_HEAD(&server->select.ios);
	return (true);
}

bool	server_close(t_server *server)
{
	socket_close(server->listen);
	return (true);
}
...
bool	call_select(int nfds, t_select *sel, int *ready)
{
	struct timeval	tv;
...
bool	server_loop(t_server *server)
{
	...
	sets_prepare(server->sets, &nfds);

	if (!call_select(nfds, &server->select, &ready))
		return (false);
	if (ready == 0)
		return (true);

	if (FD_ISSET(server->listen, &server->select.fds[RFDS]))
	{
		add_user(server);
	}

	read_msgs(&server->sets[RFDS]);
	write_msgs(&server->sets[WFDS]);

	foreach_msg_r(&server->sets[RFDS], read_msg, &nfds);
	foreach_msg_w(&server->sets[WFDS], write_msg, &nfds);

	foreach_user(server->users, nfds, check_msg_r, true);
